...
class PhotographesCarte {
  ...
  createPhotographeCarte() {
    ...
    let profilPhotographe = `
            <a class="lien-photographe" href="photographe.html?id=${
              this.photographe.id
            }" aria-label="${this.photographe.name}">
                <img alt="${
                  this.photographe.name
                }" class="img-photographe" src="ressources/img/Photographers_ID_Photos/${
      this.photographe.portrait
    }">
                <h2 class="nom-photographe">${this.photographe.name}</h2>
            </a>
            <span class="lieu-photographe">${this.photographe.city}, ${
      this.photographe.country
    }</span>
            <p class="description-photographe">
                ${this.photographe.tagline}
                    <br>
                <span class="tarif-photographe">${
                  this.photographe.price
                }€ / Jour</span>
            </p>
            <div class="liste-filtres-photographe">
                ${this.photographe.tags
                  .map(
                    (tag) =>
